...
## The WMult function finds the product of a matrix W and vector x,
## using QR and Cholesky decomposition to compute efficiently without forming W
## explicitly. The upper left p x p quadrant of W accounts for random effects
## covariance structure, and the remaining bottom right quadrant accounts for
## residual variances. Mathematically, W is given by
## (ZψZ^(T) + Iσ**2)^-1 and is a part of both terms in the log-likelihood for
## theta and beta. When taking the QR decomposition of Z, this becomes
## W = Q [(RψR^(T) + Iσ**2)^-1  &           0                 \\
...
##  W_chol: the cholesky factorisation of RψR^(T) + Iσ**2
##
## Outputs:
##  Wx: the vector which is solved for (such that Wx = y).
...
##  RSS: the evaluated residual sum of squares (y-Xβ)^T(y-Xβ), to avoid repetition.
...
lmm <- function(form, dat, ref = list()) {
  ...
  if (is.null(qrZ)) {
    ...
    # Set upper and lower bounds for θ optimisation, as we are storing 0.5 log(theta)
    # this should cover most of machine precision.
    lower <- -50
    upper <- 50
    ...
    opt_result <- optim(
      ...
      control = list(reltol = 10^(-13), maxit = 5000),
  ...
  return(list(beta = beta, theta = theta_opt, sd = exp(theta_opt)))
}
