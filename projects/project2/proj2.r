...
## only the first 150 cases provided in our data, base our results on the
...
deconv <- function(t, deaths, n.rep = 100, bs = FALSE, t0 = NULL) {
  ...
  for (rep in 1:n.rep) {
    ...
    legend("topright",
      legend = c("Incidents", if (bs) "Sampled Deaths" else "Real Deaths", "Simulated Deaths"),
      col = c("black", "red", rgb(0, 0, 1, alpha = 0.7)), # Line colours for the lines.
      lty = c(1, 1, 1) # Line types: solid, solid, solid.
    )
  }
